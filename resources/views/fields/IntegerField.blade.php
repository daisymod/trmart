@if ($action == "find")
    <div class="col-md-2">
        ...
        <input name="{{ $class->field }}[start]" type="number" class="form-control" value="{{ $value["start"] ?? "" }}">
    </div>
    <div class="col-md-2">
        <label class="col-form-label">{{ __($class->name) }} До</label>
        <input name="{{ $class->field }}[end]" type="number" class="form-control" value="{{ $value["end"] ?? "" }}">
    </div>
@else
    <div class="form-group form-field-box-{{ $class->field }}">
        <div class="form-label">{{ __($class->name) }}</div>
            <input name="{{ $class->field }}" type="number" step="1" class="form-control" value="{{ $value }}">
